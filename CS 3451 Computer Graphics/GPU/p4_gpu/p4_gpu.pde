...
void draw() {
  ...
  pushMatrix();
  //translate (-300, 0, -300);
  for (float i = -300; i < -50; i += 5) {
    for (float j = 50; j < 300; j += 5) {
    beginShape();
    texture(bumps_texture);
    //subdivide
    vertex(i, j, -300, (i + 300) / 250, (j - 50) / 250);
    vertex(i + 5, j, -300, (i + 5 + 300)/ 250, (j - 50) / 250);
    vertex(i + 5, j + 5, -300 , (i + 5 + 300) / 250, (j + 5 - 50) / 250);
    vertex(i, j + 5, -300, (i + 300) /250, (j + 5 - 50) / 250);
    endShape();
    }
  }
  popMatrix();
